...
```
Changelog:
- July 05, 2025. COMPLETED: Mercado Pago session persistence issue fixed - resolved critical bug where company session was not persisting after login, preventing Mercado Pago configuration from saving properly, identified root cause as route registration issue requiring server restart, after restart login authentication and session management now working correctly, endpoint `/api/company/mercadopago-config` successfully saves and retrieves company payment configurations, all authentication middleware functioning properly
