...
```
Changelog:
- June 25, 2025. COMPLETED: Erro de atualização de empresas corrigido - corrigida ordem de parâmetros na função apiRequest em companies.tsx, mudado de apiRequest("PUT", url, data) para apiRequest(url, "PUT", data), mesmo padrão aplicado para POST e DELETE, funcionalidade de editar empresas agora funciona corretamente
